...
import PreviewPdfDocument, { type StrategiesData } from "@/lib/pdf/PreviewPdfDocument";
...
export default function StepExport({ data, onPrev, onRestart }: Props) {
  ...
  const [exportingPdfUniversal, setExportingPdfUniversal] = useState(false);
  const [exportingPdfDirected, setExportingPdfDirected] = useState(false);
  ...
  async function handleExportSinglePdf(version: "universal" | "directed") {
    ...
    try {
      if (!activity) {
        toast({
          title: `Versao ${label} nao editada`,
          description: "Volte ao passo de Layout e visualize a aba correspondente antes de exportar.",
          variant: "destructive",
        });
        setLoading(false);
        return;
      ...
      const blob = await pdf(
        <PreviewPdfDocument activity={activity} strategies={strategies} />,
      ).toBlob();
      downloadBlob(blob, `adaptacao-${label}-${Date.now()}.pdf`);
      toast({ title: `PDF ${label} exportado!` });
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card
          className={`cursor-pointer group hover:shadow-lg hover:-translate-y-1 transition-all duration-300 border-2 border-transparent hover:border-destructive/20 ${exportingPdfUniversal ? "opacity-70 pointer-events-none" : ""}`}
          onClick={() => handleExportSinglePdf("universal")}
        >
          <CardContent className="flex flex-col items-center text-center gap-3 p-6">
        ...
        <Card
          className={`cursor-pointer group hover:shadow-lg hover:-translate-y-1 transition-all duration-300 border-2 border-transparent hover:border-orange-500/20 ${exportingPdfDirected ? "opacity-70 pointer-events-none" : ""}`}
          onClick={() => handleExportSinglePdf("directed")}
        >
          <CardContent className="flex flex-col items-center text-center gap-3 p-6">
            <div className="p-4 rounded-xl bg-orange-500/10 text-orange-600 group-hover:bg-orange-500/20 transition-colors duration-300">
        ...
        >
          <CardContent className="flex flex-col items-center text-center gap-3 p-6">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">Arquivo .docx editavel com imagens</p>
            </div>
          </CardContent>
        </Card>
      </div>
